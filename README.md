...
## About
Dataset Inspector (previously named LitData Viewer) is a desktop UI for inspecting datasets ([Lightning-AI/litData](https://github.com/Lightning-AI/litData) chunked shards and Hugging Face streaming preview).
...
## Usage
1. Download installers from [Dataset Inspector Releases](https://github.com/binbinsh/dataset-inspector/releases)
2. Browse a local `index.json` / chunk path, or a Hugging Face dataset URL, or `hf://datasets/...`, then press **Load**.
3. LitData: pick a chunk → item → field; double-click a field (or hit **Open**) to write a temp file and launch it with your default app.
4. Hugging Face: pick a split → row → field to preview values.
