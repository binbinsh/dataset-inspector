...
class SmbTransport {
  ...
  SmbTransport(
    ...
    this.sourceAddress,
    int port = 0,
    bool signingEnforced =
        false, //_signingEnforced = forceSigning || config.isSigningEnforced();
  })  : _port = port,
        _signingEnforced = signingEnforced;

  String? get lastConnectErrorMessage {
    final error = _lastConnectError;
  ...
  Future<SmbNegotiation> _negotiate() async {
    ...
    if (_port == 139) {
      ...
      _inp = SocketReader2(socket, onListenReader,
          debugPrint: config.debugPrintLowLevel, timeoutMs: config.soTimeout);
      _out = SocketWriter(socket, debugPrint: config.debugPrintLowLevel);
    }

    // Some modern servers disable SMB1 multi-protocol negotiation.
    // If minimum version is SMB2+, start with direct SMB2 negotiate.
    if (!config.forceSmb1 && config.minimumVersion.isSMB2()) {
      _smb2 = true;
    }
  ...
  Uint8List? getPreauthIntegrityHash() {
    final negotiated = _negotiated;
    if (negotiated == null) return null;
    final selected = negotiated.getSelectedDialect();
    if (selected?.atLeast(DialectVersion.SMB311) != true) {
      return null;
    }
    return Uint8List.fromList(_preauthIntegrityHash);
  }
