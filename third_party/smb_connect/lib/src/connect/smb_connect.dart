...
abstract class SmbConnect {
  ...
  static Future<bool> pingConnection({
    ...
    SmbTransport transport = SmbTransport(
      config,
      host,
  ...
  static Future<SmbConnect> connect(
    ...
    try {
      if (!await transport.ensureConnected()) {
        final detail = transport.lastConnectErrorMessage;
        if (detail != null && detail.isNotEmpty) {
          throw "Can't connect to $host: $detail";
        }
      ...
      final setupOk = await session.setup();
      if (!setupOk) {
        throw "Session setup failed for $host";
      }
  ...
  static Future<SmbConnect> connectAuth({
    ...
    if (forceSmb1) {
      return await connect(
          BaseConfiguration(
            ...
            maximumVersion: DialectVersion.SMB1,
          ),
          host,
          sourceAddress: sourceAddress,
    ...
    // SMB 3.1.1 preauth/signing paths are flaky in this Dart port.
    // Prefer SMB <= 3.0.2 first for compatibility.
    Object? legacy302Error;
    Object? smb2302Error;
    try {
      return await connect(
          BaseConfiguration(
            ...
            forceSmb1: false,
            minimumVersion: DialectVersion.SMB1,
            maximumVersion: DialectVersion.SMB302,
          ),
          host,
          sourceAddress: sourceAddress,
          onDisconnect: onDisconnect);
    } catch (error) {
      legacy302Error = error;
    }
    ...
    try {
      return await connect(
          BaseConfiguration(
            ...
            debugPrintLowLevel: debugPrintLowLevel,
            forceSmb1: false,
            minimumVersion: DialectVersion.SMB202,
            maximumVersion: DialectVersion.SMB302,
          ),
          host,
          sourceAddress: sourceAddress,
    ...
    try {
      return await connect(
          BaseConfiguration(
            ...
            forceSmb1: false,
            minimumVersion: DialectVersion.SMB1,
            maximumVersion: DialectVersion.SMB311,
          ),
          host,
          sourceAddress: sourceAddress,
          onDisconnect: onDisconnect);
    } catch (error) {
      legacy311Error = error;
    }

    throw "legacy (SMB<=3.0.2) failed: $legacy302Error; "
        "direct (SMB2..3.0.2) failed: $smb2302Error; "
        "legacy (SMB<=3.1.1) failed: $legacy311Error";
  }
