...
{
    int nbit = (int)uvar_get(ULONGSIZE);
...
{
    int i, chan, offset, limit;
    size_t nwrite = 0;
    long *data0 = data[0];
    int nchanout = ( chanout ) ? chanout : 1;
    const size_t elementSize = (size_t)(sizeout * nchanout);
 ...
 */
	...
	if( sizeout == PCM && strcmp( nativorder, outorder )) {
	    ...
	    nwrite = fwrite( &writefub[offset], elementSize, (size_t) limit, fpout);
	} else
	    nwrite = fwrite( &writebuf[offset], elementSize, (size_t) limit, fpout);
	break;
      case TYPE_S16HL:
      case TYPE_U16HL:
	if( sizeout == PCM && strcmp( nativorder, outorder )) {
...
int shortenXtract( void )
{
    ...
    int *qlpc = NULL, maxnlpc, lpcqoffset = 0, i, cmd, nscan, bitshift = 0;
    char *magic = MAGIC;
    ...
    nchan = (int)ulong_get();
    blocksize = (int)ulong_get();
    maxnlpc = (int)ulong_get();
    nmean = (int)ulong_get();
    nskip = (int)ulong_get();
    for ( i=0; i<nskip; i++ ) {
        (void)uvar_get(XBYTESIZE);
        // Non-audio skip bytes in the Shorten stream; they are intentionally ignored.
    }
    ...
    chan = 0;
    while((cmd = (int)uvar_get(FNSIZE)) != FN_QUIT)
	switch(cmd) {
	  case FN_ZERO:
	  case FN_DIFF0:
	  case FN_DIFF1:
